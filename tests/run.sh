...
scenario() {
    ...
    # Disk-throttle touchpoints: hermetic, absent by default so the feature is
    # inert unless a scenario writes a storage.cfg / sets the LVM fixtures.
    export PVE_STORAGE_CFG="$WORK/fake-storage.cfg"
    export SYS_BLOCK_BASE="$WORK/fake-sysblock"
    export DT_CALIB_FILE="$WORK/throttle-calib.json"
    mkdir -p "$SYS_BLOCK_BASE"
    # Keep the hookscript's append-log off unless a scenario opts in.
    export VCPU_PIN_LOG=""
}

# make_storage_cfg: a Proxmox storage.cfg with a dir + lvmthin (vgname pve).
make_storage_cfg() {
    cat > "$WORK/fake-storage.cfg" <<'EOF'
dir: local
	path /var/lib/vz
...
# make_vg_pvs <vg> <pv...>: register a VG's backing PV device paths.
make_vg_pvs() {
    local vg="$1"; shift
    mkdir -p "$WORK/fake-vgs"
    printf '%s\n' "$@" > "$WORK/fake-vgs/pv_name.${vg}"
    export VGS_FIXTURE_DIR="$WORK/fake-vgs"
}

# make_block_rotational <dev> <0|1>: fake /sys/block/<dev>/queue/rotational.
make_block_rotational() {
    mkdir -p "$SYS_BLOCK_BASE/$1/queue"
    echo "$2" > "$SYS_BLOCK_BASE/$1/queue/rotational"
}

# make_host_on_vg <path> <device> <vg>: host path -> device (df) -> VG (lvs).
make_host_on_vg() {
    mkdir -p "$WORK/fake-lvs"
    printf '%s %s\n' "$1" "$2" >> "$WORK/fake-df"
    printf '%s\n' "$3" > "$WORK/fake-lvs/vg_name.$(basename "$2")"
    export DF_FIXTURE="$WORK/fake-df"
...
# write_config_with_throttle <vms-json> <disk_throttle_settings-json>
write_config_with_throttle() {
    cat > "$WORK/config.json" <<EOF
{
  "global_settings": { "cpu_config_string": "host", "reserve_host_cores": true, "host_cores": [0, 8], "parallel_jobs": 2, "state_file": "manager_state.json" },
  "gpu_settings": { "required_vram_mb": 2048, "auto_detect_profile": false, "gpu_profile_map": {} },
  "vms": $1,
  "disk_throttle_settings": $2
}
EOF
}

# -----------------------------------------------------------------------------
scenario "throttle dry-run plans NVMe heuristic, equal split, mutates nothing"
make_storage_cfg
...
run_manager -f "$WORK/config.json" -n -g
assert_exit_code 0 "$RUN_RC" "exits 0"
assert_grep "$WORK/run.log" "PHASE 3.4: Planning Disk I/O Throttle" "throttle planning phase ran"
assert_grep "$WORK/run.log" "Domain vg:pve .*class=nvme source=heuristic" "domain sized by nvme heuristic"
# nvme iops_wr cap 40000 * 0.75 headroom / 4 VMs = 7500 ; iops_rd 200000*0.75/4 = 37500
assert_grep "$WORK/run.log" "\[DRY RUN\] Throttle scsi0:.*iops_wr=7500" "per-VM iops_wr computed = 7500"
assert_grep "$WORK/run.log" "\[DRY RUN\] Throttle scsi0:.*iops_rd=37500" "per-VM iops_rd computed = 37500"
# seq_wr 400MB -> 381.47MiB * 0.75 / 4 = 71.5 -> 72
assert_grep "$WORK/run.log" "\[DRY RUN\] Throttle scsi0:.*mbps_wr=72" "per-VM mbps_wr computed = 72"
assert_not_grep "$WORK/qm-calls.log" "iops_" "dry run issues NO throttle qm set"
assert_json "$WORK/manager_state.json.dryrun" '.core_assignments["101"].disk_throttle.scsi0.opts | contains("iops_wr=7500")' "state file records the throttle plan"

# -----------------------------------------------------------------------------
scenario "throttle apply writes qm set, preserves disk opts, never caps to 0"
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
write_config_with_throttle '{ "101": 2, "102": 2, "103": 2, "104": 2 }' \
  '{ "enabled": true, "weighting": "equal", "global_headroom_pct": 25, "host_reserve": { "enabled": false }, "burst": { "enabled": false }, "floors": { "mbps_rd": 10, "mbps_wr": 10, "iops_rd": 10, "iops_wr": 10 } }'
run_manager -f "$WORK/config.json" -g
assert_exit_code 0 "$RUN_RC" "exits 0"
assert_grep "$WORK/qm-calls.log" "qm set 101 -scsi0 local-lvm:vm-101-disk-0,size=32G,.*iops_wr=7500" "throttle qm set preserves volume + size="
assert_grep "$WORK/qm-calls.log" "qm set 101 -scsi0 .*mbps_wr=72" "mbps_wr applied"
assert_not_grep "$WORK/qm-calls.log" "(mbps|iops)_(rd|wr)=0([,\"]|$)" "no cap ever rounds to 0 (0 = unlimited in Proxmox)"

# -----------------------------------------------------------------------------
scenario "throttle reserves headroom when the host OS shares the disk"
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
make_host_on_vg "/" /dev/mapper/pve-root pve
write_config_with_throttle '{ "101": 2, "102": 2, "103": 2, "104": 2 }' \
  '{ "enabled": true, "weighting": "equal", "global_headroom_pct": 25, "host_reserve": { "enabled": true, "reserve_pct": 20 }, "burst": { "enabled": false }, "floors": { "mbps_rd": 10, "mbps_wr": 10, "iops_rd": 10, "iops_wr": 10 } }'
run_manager -f "$WORK/config.json" -n -g
assert_exit_code 0 "$RUN_RC" "exits 0"
assert_grep "$WORK/run.log" "Domain vg:pve .*host_shared=yes" "host-on-disk detected"
# 40000 * 0.75 headroom * 0.80 host-reserve / 4 = 6000
assert_grep "$WORK/run.log" "\[DRY RUN\] Throttle scsi0:.*iops_wr=6000" "host reserve cuts the per-VM cap (6000)"

# -----------------------------------------------------------------------------
scenario "throttle weights the split by VM core count"
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
mkdir -p "$WORK/qm-2vm"
cp "$FIXTURES/qm-basic/101.conf" "$FIXTURES/qm-basic/102.conf" "$WORK/qm-2vm/"
export QM_FIXTURE_DIR="$WORK/qm-2vm"
write_config_with_throttle '{ "101": 6, "102": 2 }' \
  '{ "enabled": true, "weighting": "cores", "global_headroom_pct": 25, "host_reserve": { "enabled": false }, "burst": { "enabled": false }, "floors": { "mbps_rd": 10, "mbps_wr": 10, "iops_rd": 10, "iops_wr": 10 } }'
run_manager -f "$WORK/config.json" -n -g
assert_exit_code 0 "$RUN_RC" "exits 0"
# weight sum 8; iops_wr pool 30000 ; 101 -> 6/8 = 22500 ; 102 -> 2/8 = 7500
assert_grep "$WORK/run.log" "VM 101 scsi0 \(weight=6\):.*iops_wr=22500" "high-core VM gets the larger slice"
assert_grep "$WORK/run.log" "VM 102 scsi0 \(weight=2\):.*iops_wr=7500" "low-core VM gets the smaller slice"

# -----------------------------------------------------------------------------
scenario "throttle counts foreign VMs in the divisor and warns, throttles only managed"
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
write_config_with_throttle '{ "101": 2 }' \
  '{ "enabled": true, "weighting": "cores", "global_headroom_pct": 25, "host_reserve": { "enabled": false }, "burst": { "enabled": false }, "floors": { "mbps_rd": 10, "mbps_wr": 10, "iops_rd": 10, "iops_wr": 10 } }'
run_manager -f "$WORK/config.json" -g
assert_exit_code 0 "$RUN_RC" "exits 0"
# 101 + 3 foreign (102/103/104, 2 cores each) => weight sum 8 ; 101 -> 2/8 of 30000 = 7500
assert_grep "$WORK/run.log" "foreign VM disk\(s\) share this disk" "foreign consumers warned"
assert_grep "$WORK/qm-calls.log" "qm set 101 -scsi0 .*iops_wr=7500" "managed VM throttled accounting for foreign share"
assert_not_grep "$WORK/qm-calls.log" "qm set 102 -scsi0 .*iops_" "foreign VM is NOT throttled"

# -----------------------------------------------------------------------------
scenario "throttle skips non-LVM (ZFS) storage with a warning"
mkdir -p "$WORK/qm-zfs"
cat > "$WORK/qm-zfs/105.conf" <<'CONF'
boot: order=scsi0
cores: 2
name: testvm-105
...
CONF
export QM_FIXTURE_DIR="$WORK/qm-zfs"
cat > "$WORK/fake-storage.cfg" <<'CONF'
zfspool: local-zfs
	pool rpool/data
	content images
CONF
export PVE_STORAGE_CFG="$WORK/fake-storage.cfg"
write_config_with_throttle '{ "105": 2 }' \
  '{ "enabled": true, "weighting": "equal", "host_reserve": { "enabled": false } }'
run_manager -f "$WORK/config.json" -n -g
assert_exit_code 0 "$RUN_RC" "exits 0"
assert_grep "$WORK/run.log" "VM 105 scsi0 .*zfspool storage -- not throttled" "ZFS disk skipped with reason"
assert_not_grep "$WORK/qm-calls.log" "iops_" "no throttle applied to ZFS"

# -----------------------------------------------------------------------------
scenario "--calibrate measures via temp LV+fio, always removes it, caches result"
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
...
assert_grep "$WORK/lvm-calls.log" "lvcreate .*--thinpool data pve" "temp thin LV created on the VG"
assert_grep "$WORK/lvm-calls.log" "lvremove -f pve/am-calib" "temp LV always removed"
assert_file_exists "$WORK/throttle-calib.json" "calibration cache written"
assert_json "$WORK/throttle-calib.json" '.["vg:pve"].iops_wr == 120000' "measured iops_wr cached"
# A subsequent run consumes the calibration (source=calibration), not heuristic.
run_manager -f "$WORK/config.json" -n -g
...
make_storage_cfg
make_vg_pvs pve /dev/nvme9n9
write_config_with_throttle '{ "101": 2, "102": 2, "103": 2, "104": 2 }' '{ "enabled": false }'
run_manager -f "$WORK/config.json" -g
assert_exit_code 0 "$RUN_RC" "exits 0"
assert_not_grep "$WORK/run.log" "PHASE 3.4: Planning Disk I/O Throttle" "throttle phase did not run"
assert_not_grep "$WORK/qm-calls.log" "iops_" "no throttle qm set issued"
